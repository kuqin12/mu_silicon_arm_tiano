...
// TODO: Hardcoded memory regions for now. This will be replaced by the SPMC
STATIC ARM_MEMORY_REGION_DESCRIPTOR MemoryTable[20];
...
// Walk through the FDT for gather all the memory regions for MMU setup
EFI_STATUS
CollectAllRegions (
  VOID* DtbAddress,
  ...
  )
{
  ...
  Offset = fdt_node_offset_by_compatible (DtbAddress, -1, "arm,ffa-manifest-1.0");
  if (Offset < 0) {
    DEBUG ((DEBUG_ERROR, "Missing FF-A boot information in manifest %d\n", Offset));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (Offset < 1) {
    DEBUG ((
          DEBUG_ERROR,
          "%a: Mis8sing in FF-A %a manifest %d\n",
          __func__,
          NodeName,
          Offset
          ));
  ...
  for (
    ...
    Property = fdt_getprop (DtbAddress, Node, "description", &LenP);
    if (Property == NULL) {
      return EFI_NOT_FOUND;
    }

    Property64 = fdt_getprop (DtbAddress, Node, "base-address", &LenP);
    if (Property64 == NULL) {
      return EFI_NOT_FOUND;
    }
    MemoryTable[*TableIndex].PhysicalBase = fdt64_to_cpu (*Property64);
    MemoryTable[*TableIndex].VirtualBase = MemoryTable[*TableIndex].PhysicalBase;

    Property32 = fdt_getprop (DtbAddress, Node, "pages-count", &LenP);
    if (Property32 == NULL) {
      return EFI_NOT_FOUND;
    }
    MemoryTable[*TableIndex].Length = fdt32_to_cpu (*Property32) * EFI_PAGE_SIZE;

    Property32 = fdt_getprop (DtbAddress, Node, "attributes", &LenP);
    if (Property32 == NULL) {
      return EFI_NOT_FOUND;
    }
    MemoryTable[*TableIndex].Attributes = fdt32_to_cpu (*Property32);

    DEBUG ((DEBUG_INFO, "Memory region description [%a]:\n", Property));
    DEBUG ((DEBUG_INFO, "    Base: 0x%llx, Size: 0x%x, Attr: 0x%x\n", MemoryTable[*TableIndex].PhysicalBase, MemoryTable[*TableIndex].Length, MemoryTable[*TableIndex].Attributes));

    MemoryTable[*TableIndex].Attributes = ARM_MEMORY_REGION_ATTRIBUTE_DEVICE;
    *TableIndex = *TableIndex + 1;

    // TODO: Hardcoded number of regions for now.
    if (*TableIndex >= 20) {
  ...
  )
{
  ...
  UINTN TableIndex = 0;
  Status = CollectAllRegions (DtbAddress, MemoryTable, "memory-regions", sizeof ("memory-regions") - 1, &TableIndex);
  ASSERT_EFI_ERROR (Status);

  Status = CollectAllRegions (DtbAddress, MemoryTable, "device-regions", sizeof ("device-regions") - 1, &TableIndex);
  ASSERT_EFI_ERROR (Status);

  MemoryTable[TableIndex].PhysicalBase = SpBootInfo.SpMemBase;
  MemoryTable[TableIndex].VirtualBase = SpBootInfo.SpMemBase;
  MemoryTable[TableIndex].Length = SpBootInfo.SpMemSize;
  MemoryTable[TableIndex].Attributes = ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK;
  TableIndex++;

  ZeroMem (&MemoryTable[TableIndex], sizeof (ARM_MEMORY_REGION_DESCRIPTOR));
