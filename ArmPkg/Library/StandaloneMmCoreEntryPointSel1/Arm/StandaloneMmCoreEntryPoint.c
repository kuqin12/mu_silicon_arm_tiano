  ...
  )
{
  ...
  while (TRUE) {
    // Exit to SPMC.
    ArmCallSmc ((ARM_SMC_ARGS*)EventCompleteSvcArgs);
    // Enter from SPMC.
  ...
  )
{
  ...
  // MMRAM related memory regions
  GuidHob.Guid = GetFirstGuidHob (&gEfiMmPeiMmramMemoryReserveGuid);
  while (GuidHob.Guid != NULL) {
    EFI_MMRAM_HOB_DESCRIPTOR_BLOCK *HobData = GET_GUID_HOB_DATA (GuidHob.Guid);
    for (UINTN Index = 0; Index < HobData->NumberOfMmReservedRegions; Index++) {
      MemoryTable[TableIndex].PhysicalBase = HobData->Descriptor[Index].PhysicalStart;
      MemoryTable[TableIndex].VirtualBase = HobData->Descriptor[Index].CpuStart;
      MemoryTable[TableIndex].Length = HobData->Descriptor[Index].PhysicalSize;
      MemoryTable[TableIndex].Attributes = ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK;
      TableIndex++;
    }
    GuidHob.Guid = GET_NEXT_HOB (GuidHob);
    GuidHob.Guid = GetNextGuidHob (&gEfiMmPeiMmramMemoryReserveGuid, GuidHob.Guid);
  }

  GuidHob.Guid = GetFirstGuidHob (&gEfiStandaloneMmNonSecureBufferGuid);

  // Non-secure communcation buffer
  EFI_MMRAM_DESCRIPTOR *NsCommBuffer = GET_GUID_HOB_DATA (GuidHob.Guid);
  if (NsCommBuffer == NULL) {
    Status = EFI_NOT_FOUND;
    DEBUG ((DEBUG_ERROR, "Error: No NsCommBuffer hob data...\n"));
    ASSERT_EFI_ERROR (Status);
  }

  MemoryTable[TableIndex].PhysicalBase = NsCommBuffer->PhysicalStart;
  MemoryTable[TableIndex].VirtualBase = NsCommBuffer->CpuStart;
  MemoryTable[TableIndex].Length = NsCommBuffer->PhysicalSize;
  MemoryTable[TableIndex].Attributes = ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK_XP_NS;
  TableIndex++;
  ...
  // Our own FV:
  MemoryTable[TableIndex].PhysicalBase = (UINTN)FvHob->BaseAddress;
  MemoryTable[TableIndex].VirtualBase = (UINTN)FvHob->BaseAddress;
  MemoryTable[TableIndex].Length = FvHob->Length;
  MemoryTable[TableIndex].Attributes = ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK;
  TableIndex++;
