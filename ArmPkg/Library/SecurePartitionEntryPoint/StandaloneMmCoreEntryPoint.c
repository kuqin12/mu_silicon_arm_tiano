...
CheckFfaCompatibility (BOOLEAN *UseOnlyFfaAbis)
{
  ...
  if (FeaturePcdGet (PcdFfaEnable) == 0) {
    return EFI_UNSUPPORTED;
  }

  // Send the SPMC our version to see whether it supports the same or not.
  SpmcVersionArgs.Arg0 = ARM_FID_FFA_VERSION;
  SpmcVersionArgs.Arg1 = ARM_FFA_CREATE_VERSION (
  ...
  if (SpmcVersion == ARM_FFA_RET_NOT_SUPPORTED) {
    return EFI_UNSUPPORTED;
  }

  // Extract the SPMC version
  SpmcMajorVer = (SpmcVersion >> ARM_FFA_MAJOR_VERSION_SHIFT) & ARM_FFA_MAJOR_VERSION_MASK;
  SpmcMinorVer = (SpmcVersion >> ARM_FFA_MINOR_VERSION_SHIFT) & ARM_FFA_MINOR_VERSION_MASK;
...
)
{
  ...
  DEBUG ((DEBUG_WARN, "Skip ns-comm buffer info for non stmm secure partitions\n"));
  DEBUG ((DEBUG_WARN, "Skip heap buffer info for non stmm secure partitions\n"));
  DEBUG ((DEBUG_WARN, "Skip shared buffer info for non stmm secure partitions\n"));
